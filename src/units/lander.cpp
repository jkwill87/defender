...
Lander::Lander(int x, int y, int z) : Unit(x, y, z) {
    ...
    get_captive = [](){return nullptr;};
    set_captive = [](Coordinate coordinate){return nullptr;};
...
void Lander::ai() {
    ...
    switch (state) {
        case SEARCHING:
            ...
            break;
        case PURSUING:
            assert_ok(captive, "no captive to pursue");
            target.x = captive->origin.x;
            target.z = captive->origin.z;
            target.y = min(captive->origin.y + MAP_CLEAR, WORLD_Y - MAP_CLEAR);
            break;
        case CAPTURING:
            assert_ok(captive, "no captive to capture");
            target.y = min(captive->origin.y + MAP_CLEAR, WORLD_Y - MAP_CLEAR);
            break;
        case ESCAPING:
            assert_ok(captive, "no captive to escape with");
            if (cycle % 10 == 0) {
                ++target.y;
                captive->action_lift();
            }
            break;
        case EXITED:
            assert_ok(captive, "no captive to exit with");
...
string Lander::as_str() {
    return"lander id: " + to_string(id);
}
